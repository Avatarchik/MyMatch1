...
//	        AudioAssistant.Shot("CrossBombCrush");
			...
			FieldAssistant.Instance.BlockCrush(sx, sy, false);
...
//				DebugUtil.Info("==== explode begin:" + s.key + "," + s.transform.localPosition);
		...
		public List<Card> GetDangeredChips(List<Card> stack) {
			if (stack.Contains(chip) || !chip.parentSlot || FieldAssistant.Instance.field == null)
	            return stack;
			...
			for (int x = 0; x < FieldAssistant.Instance.field.width; x++) {
	            if (x == sx)
	                continue;
				s = SlotManager.Instance.FindSlot(x, sy);
			...
			for (int y = 0; y < FieldAssistant.Instance.field.height; y++) {
	            if (y == sy)
	                continue;
				s = SlotManager.Instance.FindSlot(sx, y);
	            if (s && s.GetChip()) {
	                stack = s.GetChip().GetDangeredChips(stack);
	            }
	        }
