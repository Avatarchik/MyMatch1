...
namespace FightNew
{
	...
	public class ThreeLineCard : IHelpCard 
	{
...
//			{
//				if(Utils.straightSides[i] != FlyOperation)
//				{
//					Destroy(transform.FindChild(Utils.GetSideName(Utils.straightSides[i])).gameObject);
//				}	
//			}
//
//			anim.Play("CrossBump");
			...
			for(int i = 0;i < Utils.allSides.Length;i++)
			{
				if(!FlyOperation.Contains(Utils.allSides[i]))
				{
					...
					Destroy(transform.FindChild(Utils.GetSideName(Utils.allSides[i]) + "2").gameObject);
					Destroy(transform.FindChild(Utils.GetSideName(Utils.allSides[i]) + "3").gameObject);
				}	
			}
